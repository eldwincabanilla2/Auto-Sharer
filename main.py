...
import time
init()
colors = [Fore.WHITE, Fore.YELLOW, Fore.CYAN, Fore.GREEN, Fore.MAGENTA, Fore.RED, Fore.BLUE]
config = {
  'cookies': '',
  'id': ''
}
...
banner(texx)
config['cookies'] = input("\nEnter Cookies: ")
config['id'] = input("\nEnter Link: ")
share_count = int(input("\nEnter Amount: "))
import os
os.system("clear")
banner(texx)
headers = {
  ...
  async def share(self, session, token, cookie):
    ...
    while count < share_count:
      async with session.post(f'https://graph.facebook.com/me/feed?link=https://m.facebook.com/{config["id"]}&published=0&access_token={token}', headers=headers) as response:
        ...
        # count += 1
        if 'id' in data:
          print(f"[ {count}/{share_count} ] - {Fore.GREEN}{data['id']} - OK{Style.RESET_ALL}",end="\r")
          count += 1
        else:
          # print(data)
